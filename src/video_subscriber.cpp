...
private:
    ...
    void initializeDataLogging() {
        ...
        std::stringstream ss;
        ss << output_dir_ << "/driving_log_" << std::put_time(std::localtime(&time_t), "%Y%m%d_%H%M%S") << ".csv";
    ...
    void initializeVideoWriter(const cv::Mat &first_frame) {
        ...
        if (filename_.empty()) {
            ...
            auto time_t = chrono::system_clock::to_time_t(now);
            std::stringstream ss;
            ss << output_dir_ << "/recording_" << std::put_time(std::localtime(&time_t), "%Y%m%d_%H%M%S") << ".avi";
            filename_ = ss.str();
        }
    ...
    void drawEnhancedHUD(cv::Mat &image, float ego_speed, int acc_speed, int max_speed,
        ...
        // Background for HUD
        cv::rectangle(image, cv::Point(10, 10), cv::Point(400, 250), cv::Scalar(0, 0, 0, 128), -1);
        ...
        // Speed information
        cv::putText(image, "Speed Control:", cv::Point(20, 65), 
                   font, font_scale, cv::Scalar(255, 255, 255), thickness);
        cv::putText(image, "Ego: " + std::to_string((int)ego_speed) + " km/h", 
                   cv::Point(20, 85), font, font_scale, cv::Scalar(0, 255, 0), thickness);
        cv::putText(image, "Target: " + std::to_string(acc_speed) + " km/h", 
                   cv::Point(20, 105), font, font_scale, cv::Scalar(255, 255, 0), thickness);
        
        if (max_speed != -1) {
            cv::putText(image, "Limit: " + std::to_string(max_speed) + " km/h", 
                       cv::Point(20, 125), font, font_scale, cv::Scalar(255, 0, 0), thickness);
        }
        
        // Action and target information
        cv::putText(image, "Action: " + action_str, cv::Point(20, 155), 
                   font, font_scale, action_color, thickness);
        cv::putText(image, "Target ID: " + std::to_string(target_id), 
                   cv::Point(20, 175), font, font_scale, cv::Scalar(255, 255, 255), thickness);
        
        // Performance metrics
        cv::putText(image, "FPS: " + std::to_string((int)fps), 
                   cv::Point(20, 195), font, font_scale, cv::Scalar(255, 255, 255), thickness);
        cv::putText(image, "Detections: " + std::to_string(detection_count), 
                   cv::Point(20, 215), font, font_scale, cv::Scalar(255, 255, 255), thickness);
        cv::putText(image, "Proc Time: " + std::to_string((int)(avg_processing_time_ * 1000)) + "ms", 
                   cv::Point(20, 235), font, font_scale, cv::Scalar(255, 255, 255), thickness);
        
        // Safety indicators
        if (emergency_stop_) {
            cv::rectangle(image, cv::Point(image.cols - 150, 10), 
                         cv::Point(image.cols - 10, 50), cv::Scalar(0, 0, 255), -1);
        ...
        if (front_speed > 0) {
            cv::putText(image, "Front Speed: " + std::to_string((int)front_speed) + " km/h", 
                       cv::Point(200, 105), font, font_scale, cv::Scalar(255, 255, 255), thickness);
        }
    }
...
private:
    ...
    void updateSpeedLimits(const std::vector<STrack>& outputStracks) {
        // Enhanced speed limit detection
        for (const STrack &obj : outputStracks) {
            int classId = obj.classId;
            ...
            if (classId >= 12 && classId <= 17 && conf > CONFIDENCE_THRESHOLD) {
                ...
                if (detectedSpeed != maxSpeed_) {
                    maxSpeed_ = detectedSpeed;
                    if (enable_debug_output_) {
                        ROS_INFO("Speed limit updated: %d km/h", maxSpeed_);
                    }
                }
            }
        }
        
        // Enhanced speed control logic
        if (maxSpeed_ != -1) {
