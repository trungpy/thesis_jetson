...
FrontDistanceEstimator::FrontDistanceEstimator(double focalLength,
                                               double realObjectWidth)
    : m_focalLength(focalLength), m_realObjectWidth(realObjectWidth) {
    std::cout << "FrontDistanceEstimator initialized with focal length: "
              << m_focalLength
...
double FrontDistanceEstimator::estimate(double pixelDistance) {
    if (this->m_focalLength <= 0 || this->m_realObjectWidth <= 0) {
        throw std::invalid_argument(
            "Focal length and real object width must be positive.");
    }
